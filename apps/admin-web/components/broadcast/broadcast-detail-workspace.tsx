...
function EvidenceSections({ broadcastId, items, title = "Broadcast evidence", description }: { broadcastId: string; items: BroadcastDetailView["attachments"]; title?: string; description?: string }) {
  return (
...
function AdminBroadcastDetails({ broadcast }: { broadcast: BroadcastDetailView }) {
  ...
  return <>
    <BroadcastInformation broadcast={broadcast} />
    <EvidenceSections broadcastId={broadcast.id} items={media.evidence} title="Attachments" description="Only shown when the admin attached supporting material." />
    <BroadcastDetailMap title="Target location" description={`Scope: ${broadcast.target}`} {...mapProps(broadcast)} showOpenLocation={false} />
  </>;
}

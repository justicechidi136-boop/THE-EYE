...
export function CommunityAudioEvidencePlayer({ postId, media }: Props) {
  ...
  async function loadPlaybackUrl() {
    ...
    try {
      const response = await fetch(
        `/api/admin/neighborhood-watch/posts/${postId}/media/${media.id}/voice`,
      );
  ...
  return (
    <div className="mt-3 rounded-lg border border-line bg-surface p-3">
      <p className="text-xs font-semibold uppercase text-muted">Community voice post</p>

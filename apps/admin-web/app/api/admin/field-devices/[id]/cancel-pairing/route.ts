...
import { cancelFieldDevicePairing } from "../../../../../../lib/api/data";
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const result = await cancelFieldDevicePairing(id, body);
    return NextResponse.json({ ok: true, data: result });
  } catch (error) {
    return errorResponse(error, "Failed to cancel pairing");
  }
}

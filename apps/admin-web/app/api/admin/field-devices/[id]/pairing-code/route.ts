...
import { issueFieldDevicePairingCode } from "../../../../../../lib/api/data";
...
export async function POST(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    const pairing = await issueFieldDevicePairingCode(id, body);
    return NextResponse.json({ ok: true, data: pairing });
  } catch (error) {
    return errorResponse(error, "Failed to issue pairing code");
  }
}

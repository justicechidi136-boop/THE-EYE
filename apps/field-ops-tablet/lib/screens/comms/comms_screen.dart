...
class _CommsScreenState extends State<CommsScreen> {
  ...
  Future<void> _loadComms(String assignmentId) async {
    ...
    try {
      await widget.services.restoreSession();
      final assignment = await widget.services.workflows.getAssignment(assignmentId);
      final incident = Map<String, dynamic>.from(assignment['incident'] as Map? ?? const {});
      final incidentId = incident['id']?.toString();
      if (incidentId == null || incidentId.isEmpty) {
        throw FieldApiException('Assignment has no incident scope');
      }
      final messages = await widget.services.workflows.listIncidentMessages(incidentId);
      if (!mounted) return;
      setState(() {
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            return ListTile(
                              ...
                              title: Text(
                                incident['title']?.toString() ?? 'Assignment $id',
                              ),
                    ...
                    children: [
                      if (_error != null)
                        Padding(
                          ...
                          child: Text(_error!, style: const TextStyle(color: FieldColors.danger)),
                        ),
                      Expanded(
                        child: _busy
                            ? const Center(child: CircularProgressIndicator())
                            ...
                            OutlinedButton(
                              onPressed: () {
                                Navigator.of(context).pushNamed(
                                  ...
                                  arguments: {'assignmentId': _selectedAssignmentId},
                                );
                              },
...
class _CommsList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty) {
      return const Center(child: Text('No messages in incident scope'));
    }
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          leading: Icon(
            row['messageType'] == 'Voice' ? Icons.mic : Icons.chat_bubble_outline,
          ),
          title: Text(row['body']?.toString() ?? row['messageType']?.toString() ?? 'Message'),
          subtitle: Text(
            '${row['senderRole'] ?? 'Unknown'} · ${row['createdAt'] ?? ''}',
          ),
          trailing: row['readAt'] == null
              ? const Icon(Icons.mark_email_unread, color: FieldColors.orange, size: 18)

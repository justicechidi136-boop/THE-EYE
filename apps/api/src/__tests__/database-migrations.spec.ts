...
describe("database migrations", () => {
  ...
  it("declares incident media GPS as generated by its database trigger", () => {
    const schema = readFileSync(schemaPath, "utf8");
    const initialMigration = readFileSync(
      join(
        migrationsDir,
        "20260705180000_initial_public_safety_schema",
        "migration.sql",
      ),
      "utf8",
    );
    ...
    expect(schema).toMatch(
      /model IncidentMedia[\s\S]*gpsLocation\s+Unsupported\("geography\(Point,4326\)"\)\s+@default\(dbgenerated\(\)\)/,
    );
    expect(initialMigration).toContain(
      "CREATE TRIGGER trg_incident_media_set_gps",
    );
  });
});

...
async function main() {
  if (process.env.NODE_ENV === "production") {
    console.error("Database seeding is blocked in production");
    process.exit(1);
  }

  const devAdminEmail = process.env.ADMIN_EMAIL ?? "dev-admin@theeye.local";
  const devAdminPassword = process.env.ADMIN_PASSWORD ?? "change_me_dev_admin_password";
  const seedPasswordHash = hashPassword(devAdminPassword);
  ...
  const seedSql = async (sql: string) => {
  ...
  await seedSql(`
    INSERT INTO admin_users (id, role_id, agency_id, jurisdiction_id, email, password_hash, display_name, country, state, lga, is_active) VALUES
      ('${superAdminUserId}', '${adminRoleId}', NULL, '${jurisdictionId}', '${devAdminEmail}', '${seedPasswordHash}', 'THE EYE Super Admin', 'Nigeria', 'Lagos', 'Ikeja', true),
      ('${adminUserId}', '${dispatcherRoleId}', '${policeAgencyId}', '${jurisdictionId}', 'dispatcher.ikeja@theeye.local', '${seedPasswordHash}', 'Ikeja Dispatcher', 'Nigeria', 'Lagos', 'Ikeja', true)
